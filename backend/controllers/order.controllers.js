...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        if (status == "out of delivery" && !shopOrder.assignment) {
            ...
            // Fallback: if no delivery boy is found nearby, get all delivery boys
            if (nearByDeliveryBoys.length === 0) {
                nearByDeliveryBoys = await User.find({ role: "deliveryBoy" });
            }

            // Remove duplicates (in case nearby query already found some)
            const allDeliveryBoys = await User.find({ role: "deliveryBoy" });
            const nearbyIds = new Set(nearByDeliveryBoys.map(b => String(b._id)));
            allDeliveryBoys.forEach(b => {
                if (!nearbyIds.has(String(b._id))) {
                    nearByDeliveryBoys.push(b);
                }
            });
...
export const sendDeliveryOtp = async (req, res) => {
    try {
        ...
        try {
            ...
            console.log("mail send failed, OTP still generated:", mailErr.message)
        }
        return res.status(200).json({ message: `Otp sent Successfuly to ${order?.user?.fullName}`, otp })
    } catch (error) {
